---
title:
date: 2022-10-24
type: landing

sections:
  - block: hero
    content:
      title: |
        Juanes Lab
      image:
        filename: mari-angeles-juanes.jpg
      text: |
        <div class="jl-hero-eyebrow">Cytoskeleton &middot; Cell Migration &middot; Cancer Invasion</div>
        <div class="jl-hero-rule">
          <p class="jl-hero-tagline">Research group led by Dr María Ángeles Juanes at the Centro de Investigación Príncipe Felipe (CIPF), Valencia, Spain</p>
        </div>
        <div class="jl-hero-ctas">
          <a href="#research" class="jl-btn jl-btn-primary">Explore our research</a>
          <a href="{{< relurl "/people/" >}}" class="jl-btn jl-btn-ghost">Meet the team</a>
        </div>

  - block: markdown
    id: about
    content:
      title: About us
      text: |
        The Juanes Lab studies how the cytoskeleton is remodeled during collective cell migration and cancer invasion — combining techniques from single-molecule biochemistry to advanced live-cell and super-resolution imaging, in models ranging from reconstituted proteins to mice.

        **Our research objectives**

        - Understanding the mechanisms underlying cytoskeleton remodeling events in collective cell migration, cancer invasion and metabolism.
        - Identifying novel genetic signatures and/or biomarkers for the early detection of human cancers, to ultimately improve cancer diagnosis and assist in personalized therapy.
    design:
      columns: '1'
      spacing:
        padding: ['48px', '0', '32px', '0']

  - block: markdown
    id: research
    content:
      title: Research
      subtitle: Four lines of research, one shared goal
      text: |
        <div class="jl-card-grid">
          <a class="jl-card" href="{{< relurl "/research/cell-migration-invasion/" >}}">
            <div class="jl-card-media"><img src="{{< relurl "/media/research-migration.png" >}}" alt="Confocal microscopy image of a cell spheroid used to study collective migration and invasion" loading="lazy" /></div>
            <div class="jl-card-body">
              <span class="jl-card-num">01</span>
              <h3>Cell migration and invasion</h3>
              <p>How the cytoskeleton controls cell adhesion and coordination during collective migration, and how this is subverted during cancer invasion.</p>
              <span class="jl-card-link">Learn more →</span>
            </div>
          </a>
          <a class="jl-card" href="{{< relurl "/research/early-diagnosis/" >}}">
            <div class="jl-card-media"><img src="{{< relurl "/media/research-diagnosis.png" >}}" alt="Confocal microscopy image of cancer cells used in biomarker research" loading="lazy" /></div>
            <div class="jl-card-body">
              <span class="jl-card-num">02</span>
              <h3>Early diagnosis</h3>
              <p>Novel genetic signatures and biomarkers for the early detection of human cancers, to improve diagnosis and personalized therapy.</p>
              <span class="jl-card-link">Learn more →</span>
            </div>
          </a>
          <a class="jl-card" href="{{< relurl "/research/drug-discovery/" >}}">
            <div class="jl-card-media"><img src="{{< relurl "/media/research-drugdiscovery.png" >}}" alt="Molecular docking model of a candidate anticancer compound" loading="lazy" /></div>
            <div class="jl-card-body">
              <span class="jl-card-num">03</span>
              <h3>Drug discovery</h3>
              <p>AI-assisted, structure-based discovery of anticancer compounds, from in silico prediction to validation in cellular and animal models.</p>
              <span class="jl-card-link">Learn more →</span>
            </div>
          </a>
          <a class="jl-card" href="{{< relurl "/research/cell-metabolism/" >}}">
            <div class="jl-card-media"><img src="{{< relurl "/media/research-metabolism.png" >}}" alt="Confocal microscopy image showing mitochondria in cultured cells" loading="lazy" /></div>
            <div class="jl-card-body">
              <span class="jl-card-num">04</span>
              <h3>Cell metabolism</h3>
              <p>How the actin cytoskeleton relates to metabolic plasticity, and how cells respond energetically to stress.</p>
              <span class="jl-card-link">Learn more →</span>
            </div>
          </a>
        </div>
        <p class="jl-note"><a href="{{< relurl "/research/" >}}">See the full Research page →</a></p>
    design:
      columns: '1'

  - block: markdown
    id: texture-divider
    content:
      title:
      text: |
        <p class="jl-texture-caption">Actin cytoskeleton, confocal microscopy — Juanes Lab</p>
    design:
      columns: '1'
      background:
        image:
          filename: cytoskeleton-texture.png
          filters:
            brightness: 0.55
          parallax: false
          position: center
          size: cover
      spacing:
        padding: ['26px', '0', '26px', '0']

  - block: markdown
    id: cipf
    content:
      title:
      text: |
        <div class="jl-scrim">
          <p class="jl-eyebrow">Based at CIPF</p>
          <p>Centro de Investigación Príncipe Felipe, Valencia — Valencia Biomedical Research Foundation</p>
        </div>
    design:
      columns: '1'
      background:
        image:
          filename: cipf-building.jpg
          filters:
            brightness: 0.55
          parallax: false
          position: center
          size: cover
          text_color_light: true
      spacing:
        padding: ['130px', '0', '130px', '0']

  - block: markdown
    id: testimonials
    content:
      title: What people say
      text: |
        <div class="jl-testimonials">
          <div class="jl-testimonial">
            <span class="jl-quote-mark">&ldquo;</span>
            <p class="jl-testimonial-text">Mari Angels was a colleague of mine during my PhD in CNRS Montpellier. She was a very creative, intellectually outstanding and hard-working scientist, efficiently addressing challenges and meeting goals. She is really focused on her tasks, well-organized, and very committed. She's been a great colleague to work with, as she is a naturally amicable and open-minded person. She has also proven to be an excellent team player, as Mari Angels is a good listener, very supportive and goes out of her way to help people with her scientific knowledge and problem-solving skills.</p>
            <p class="jl-testimonial-name">Dr Ilaria Scarfone</p>
            <p class="jl-testimonial-role">Specialist of Pharma Analytics, Thermo Fisher Scientific</p>
          </div>
          <div class="jl-testimonial-sep"></div>
          <div class="jl-testimonial">
            <span class="jl-quote-mark">&ldquo;</span>
            <p class="jl-testimonial-text">I have known Mari Angeles from long time ago, we have done the PhD together. She is extremely dedicated to her scientific career. Her passion for science never ceases to amaze me. She has an endeavor desire to achieve excellence in all that she undertakes. Mari Angeles is one of the most dynamic, hard working, detail-oriented and perseverant person that I have the pleasure of knowing. She is open, very kind person and ready to give professional and friendly advice drawn upon her academic experience.</p>
            <p class="jl-testimonial-name">Maria Soriano Carot</p>
            <p class="jl-testimonial-role">Vice President, Business Strategist and Communication Director at IMFAHE</p>
          </div>
          <div class="jl-testimonial-sep"></div>
          <div class="jl-testimonial">
            <span class="jl-quote-mark">&ldquo;</span>
            <p class="jl-testimonial-text">As PhD mentor of Mari Angels I would say that she did an outstanding job. She is extremely hard worker, shows incredible enthusiasm for research and has the ability to quickly understand entangled mechanisms and troubleshoot whatever she found in her way. As a person, she got along wonderfully with everybody. She was the most precious part of the laboratory and a real team player.</p>
            <p class="jl-testimonial-name">Carmen Bañó Aracil</p>
            <p class="jl-testimonial-role">Catedrática de Bioquímica, Universitat de València</p>
          </div>
        </div>
    design:
      columns: '1'

  - block: markdown
    id: team-photo
    content:
      title:
      text: |
        <div class="jl-photo-caption">
          <p>Members of the Juanes Lab at CIPF, Valencia.</p>
        </div>
    design:
      columns: '1'
      background:
        image:
          filename: lab-group.jpg
          filters:
            brightness: 0.85
          parallax: false
          position: center
          size: cover
          text_color_light: true
      spacing:
        padding: ['110px', '0', '110px', '0']

  - block: markdown
    id: contact
    content:
      title:
      text: |
        <div class="jl-footer">
          <h3 class="jl-footer-title">Get in touch</h3>
          <p class="jl-footer-line"><a href="mailto:majuanes@cipf.es">majuanes@cipf.es</a> &middot; +34 963289680 ext. 2116</p>
          <p class="jl-footer-line">CIPF — Centro de Investigación Príncipe Felipe<br>C/ Eduardo Primo Yúfera, 3 · 46012 Valencia, Spain</p>
          <div class="jl-footer-brand">
            <img src="{{< relurl "/media/juanes-lab-logo.png" >}}" alt="Juanes Lab" class="jl-footer-logo" />
            <img src="{{< relurl "/media/cipf-logo.png" >}}" alt="CIPF — Valencia Biomedical Research Foundation" class="jl-footer-cipf" />
            <img src="{{< relurl "/media/juaneslab-qr.png" >}}" alt="QR code to juaneslab.com" class="jl-footer-qr" />
          </div>
        </div>
    design:
      columns: '1'
      background:
        color: 'black'
      spacing:
        padding: ['64px', '0', '48px', '0']
---
